/**
 * GET/DELETE /api/admin/exams/[examId]
...
export default async function handler(
  ...
  if (method === 'DELETE') {
    try {
      ...
      if (examError) {
        return res.status(500).json({ success: false, error: '시험 삭제 실패', details: examError.message });
      }

      return res.status(200).json({ success: true, message: '시험이 삭제되었습니다' });
    } catch (error: any) {
      console.error('[ExamDelete] Error:', error);
